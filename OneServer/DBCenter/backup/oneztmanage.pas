...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        QOneDataResult.ResultMsg :=
          '第' + (i + 1).ToString + '个数据集保存数据集时,保存的表名不可为空';
        exit;
      end;
      if lDataSaveDML.Primarykey = '' then
      begin
        QOneDataResult.ResultMsg :=
          '第' + (i + 1).ToString + '个数据集保存数据集时,保存的主键不可为空';
        exit;
      end;
      if lDataSaveDML.SaveData = '' then
      begin
        QOneDataResult.ResultMsg :=
          '第' + (i + 1).ToString + '个数据集提交的数据为空,请检查';
        exit;
      end;
    end
    ...
    begin
      ...
      begin
        QOneDataResult.ResultMsg :=
          '第' + (i + 1).ToString + '个数据集执行DML操作语句时,操作语句不可为空';
        exit;
      end;
    end
    else
    begin
      QOneDataResult.ResultMsg :=
        '第' + (i + 1).ToString + '个数据集未知的提交模式' + lDataSaveDML.DataSaveMode;
      exit;
    end;
        ...
        begin
          ...
          begin
            ...
            begin
              if lZTItem.FException.FErrmsg <> '' then
                QOneDataResult.ResultMsg :=
                  '保存出现异常:异常原因:' + lZTItem.FException.FErrmsg;
              exit;
            end;
          end
          else if lDataSaveDML.DataSaveMode = const_DataSaveMode_SaveDML then
          begin
            ...
            begin
              ...
              begin
                QOneDataResult.ResultMsg :=
                  '执行DML语句异常,原因:必需影响行数[' + lDataSaveDML.AffectedMustCount.ToString + '],当前影响行数[' + LZTQuery.RowsAffected.ToString + ']';
                exit;
              end;
            end;
            ...
            begin
              ...
              begin
                QOneDataResult.ResultMsg :=
                  '执行DML语句异常,原因:最大影响行数[' + lDataSaveDML.AffectedMaxCount.ToString + '],当前影响行数[' + LZTQuery.RowsAffected.ToString + ']';
                exit;
              end;
            end;
          end;
...
begin
  ...
  begin
    ...
    lMakeExecSQL := 'set @OneZSysMakeSQL='; //set @OneZSysMakeSQL=lMakeExecSQL 后面在组装
