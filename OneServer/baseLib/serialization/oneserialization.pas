...
procedure AddListClass(QListClass: TClass; QListItemClass: TClass;
  QItemCreateEven: evenListItemCreate = nil);
...
//把一个JSON序列化成对象
function JsonToObject(const QSource: TObject; const QJsonData: TJsonData;
  var QErrMsg: string): boolean;
//判断对象是泛型，还是容器类型，还是map字典
function IsGenericsCollections(const QSource: TObject; QListRttiType: TRttiType;
  var QemList: emListT; var QListClass: TOneListClass): boolean;
...
begin
  ...
  begin
    QErrMsg := '无法序列化对象类型' +
      GetEnumName(TypeInfo(TTypeKind), Ord(LRttiType.TypeKind));
    Exit;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else if LArrTValue.Kind in [tkString, tkAString, tkChar,
              tkLString, tkUChar, tkUString, tkVariant] then
    ...
    begin
      ...
      case LemListT of
        ...
        begin
          ...
          begin
            QErrMsg := LRttiType.AsInstance.MetaClassType.ClassName +
              '对像未注册,请和OneSerialization.AddListClass注册';
            exit;
          end;
          if (LListClass.FListItemClass = nil) and
            (LListClass.FItemCreateEven = nil) then
          begin
            QErrMsg := LRttiType.AsInstance.MetaClassType.ClassName +
              '对像已注册,但TList<T>,T类型未注册';
            exit;
          end;
          lListTObject := TList<TObject>(QSource);
          //获取T的RTTI信息,然后动态生成对象
          ...
          begin
            ...
            JsonToObject(lListItemObj, lJsonArray[i], QErrMsg);
  ...
  begin
    ...
    case LPropRttiType.TypeKind of
      ...
      begin
        ...
        JsonToObject(tempObj, tempJsonData, QErrMsg);
      end;
      tkSString, tkLString, tkAString, tkWString, tkWChar, tkUString, tkUChar:
      begin
...
begin
  ...
  if (lClassName = 'tlist<string>') or (lClassName = 'tlist<ansistring>') or
    (lClassName = 'tlist<widestring>') then
  begin
